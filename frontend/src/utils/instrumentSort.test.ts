...
describe("sortByInstrument", () => {
  describe("弦楽器の順序", () => {
    it("vn → va → vc → cb の順に並ぶ", () => {
      ...
      expect(result).toEqual(["Vn いちろう", "Vaたろう", "Vc はなこ", "次郎(CB)"]);
    });
  });
  ...
  describe("管楽器の順序", () => {
    it("fl → ob → cl → hr → tp → trb → tuba の順に並ぶ", () => {
      const result = sorted(
        "やまもとチューバ", "たなか Trp", "Hrさとう", "CLけんじ", "Ob やまだ", "FL もり", "Tb すずき"
      );
      expect(result).toEqual([
        "FL もり", "Ob やまだ", "CLけんじ", "Hrさとう", "たなか Trp", "Tb すずき", "やまもとチューバ",
      ]);
    });
  });
  ...
  describe("大文字小文字の無視", () => {
    ...
    it("FL / Fl / fl をすべて同じ楽器として扱う", () => {
      ...
      expect(withOb.indexOf("Ob やまだ")).toBeGreaterThan(withOb.indexOf("FL もり"));
    });
  });
  ...
  describe("特殊なケース", () => {
    ...
    it("日本語楽器名を認識する (打楽器, たいこ, チューバ)", () => {
      const result = sorted("打楽器　まつもと", "たいこかわい", "なかむらチューバ");
    ...
    it("vla という表記を va として認識する", () => {
      const result = sorted("Vn いちろう", "Vla たかむら", "vla.むらかみ", "Vc はなこ");
      expect(result[0]).toBe("Vn いちろう");
    ...
    it("楽器不明は末尾に集まる", () => {
      ...
      expect(result.indexOf("まめこ")).toBeGreaterThan(result.indexOf("Vn いちろう"));
    });
  });
});
