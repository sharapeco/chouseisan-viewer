...
describe("parseChouseisan", () => {
	...
	describe("participants", () => {
		it("日程行のヘッダーから参加者名を返す", () => {
			expect(parseChouseisan(BASIC).participants).toEqual([
				"田中",
				"鈴木",
				"佐藤",
			]);
	...
	describe("回答マッピング", () => {
		...
		it("△ を 'delta' にマップする", () => {
			expect(parseChouseisan(ANSWER_VARIANTS).dateRows[0].answers[5]).toBe(
				"delta",
			);
		});
	...
	describe("comments", () => {
		it("コメント行の内容を参加者順に返す", () => {
			expect(parseChouseisan(BASIC).comments).toEqual([
				"よろしく",
				"",
				"都合次第",
			]);
